import Config

# config/runtime.exs is executed for all environments, including
# during releases. It is executed after compilation and before the
# system starts, so it is typically used to load production configuration
# and secrets from environment variables or elsewhere. Do not define
# any compile-time configuration in here, as it won't be applied.
# The block below contains prod specific runtime configuration.

# ## Using releases
#
# If you use `mix release`, you need to explicitly enable the server
# by passing the PHX_SERVER=true when you start it:
#
#     PHX_SERVER=true bin/agent_lens start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :agent_lens, AgentLensWeb.Endpoint, server: true
end

config :agent_lens, AgentLensWeb.Endpoint,
  http: [port: String.to_integer(System.get_env("PORT", "4000"))]

# ── LangSmith ────────────────────────────────────────────────────────────────
#
# Agent identity is the LangSmith *workspace*: each agent traces into its own
# workspace, so the workspace id is the dimension every KPI slices by. One
# org-scoped API key covers every workspace; a per-workspace key can be layered
# on later without reshaping this config.
#
# The client implementation is chosen here rather than at compile time so that
# dev and test run against the mock with no API key and no network access.

langsmith_client =
  case System.get_env("LANGSMITH_CLIENT") do
    "http" ->
      AgentLens.LangSmith.HTTP

    "mock" ->
      AgentLens.LangSmith.Mock

    nil ->
      if config_env() == :prod, do: AgentLens.LangSmith.HTTP, else: AgentLens.LangSmith.Mock

    other ->
      raise """
      invalid LANGSMITH_CLIENT: #{inspect(other)}

      Expected "http" or "mock".
      """
  end

# Fail fast on a missing key, but only when we actually intend to call
# LangSmith. An unconditional fetch_env! here would break `mix test` and dev,
# which run against the mock by design.
langsmith_api_key =
  if langsmith_client == AgentLens.LangSmith.HTTP do
    System.fetch_env!("LANGSMITH_API_KEY")
  end

# Comma-separated workspace ids, one per agent.
langsmith_workspaces =
  "LANGSMITH_WORKSPACES"
  |> System.get_env("")
  |> String.split(",", trim: true)
  |> Enum.map(&String.trim/1)
  |> Enum.reject(&(&1 == ""))

if langsmith_client == AgentLens.LangSmith.HTTP and langsmith_workspaces == [] do
  raise """
  environment variable LANGSMITH_WORKSPACES is missing.

  Agent identity comes from the LangSmith workspace, so at least one workspace
  id is required. For example: LANGSMITH_WORKSPACES=ws-support,ws-research
  """
end

config :agent_lens, :langsmith,
  client: langsmith_client,
  endpoint: System.get_env("LANGSMITH_ENDPOINT", "https://api.smith.langchain.com"),
  api_key: langsmith_api_key,
  workspaces: langsmith_workspaces

if config_env() == :dev do
  # Reload browser tabs when matching files change.
  config :agent_lens, AgentLensWeb.Endpoint,
    live_reload: [
      web_console_logger: true,
      patterns: [
        # Static assets, except user uploads
        ~r"priv/static/(?!uploads/).*\.(js|css|png|jpeg|jpg|gif|svg)$",
        # Router, Controllers, LiveViews and LiveComponents
        ~r"lib/agent_lens_web/router\.ex$",
        ~r"lib/agent_lens_web/(controllers|live|components)/.*\.(ex|heex)$"
      ]
    ]
end

if config_env() == :prod do
  database_url =
    System.get_env("DATABASE_URL") ||
      raise """
      environment variable DATABASE_URL is missing.
      For example: ecto://USER:PASS@HOST/DATABASE
      """

  maybe_ipv6 = if System.get_env("ECTO_IPV6") in ~w(true 1), do: [:inet6], else: []

  config :agent_lens, AgentLens.Repo,
    # ssl: true,
    url: database_url,
    pool_size: String.to_integer(System.get_env("POOL_SIZE") || "10"),
    # For machines with several cores, consider starting multiple pools of `pool_size`
    # pool_count: 4,
    socket_options: maybe_ipv6

  # The secret key base is used to sign/encrypt cookies and other secrets.
  # A default value is used in config/dev.exs and config/test.exs but you
  # want to use a different value for prod and you most likely don't want
  # to check this value into version control, so we use an environment
  # variable instead.
  secret_key_base =
    System.get_env("SECRET_KEY_BASE") ||
      raise """
      environment variable SECRET_KEY_BASE is missing.
      You can generate one by calling: mix phx.gen.secret
      """

  host = System.get_env("PHX_HOST") || "example.com"

  config :agent_lens, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")

  config :agent_lens, AgentLensWeb.Endpoint,
    url: [host: host, port: 443, scheme: "https"],
    http: [
      # Enable IPv6 and bind on all interfaces.
      # Set it to  {0, 0, 0, 0, 0, 0, 0, 1} for local network only access.
      # See the documentation on https://bandit.hexdocs.pm/Bandit.html#t:options/0
      # for details about using IPv6 vs IPv4 and loopback vs public addresses.
      ip: {0, 0, 0, 0, 0, 0, 0, 0}
    ],
    secret_key_base: secret_key_base

  # ## SSL Support
  #
  # To get SSL working, you will need to add the `https` key
  # to your endpoint configuration:
  #
  #     config :agent_lens, AgentLensWeb.Endpoint,
  #       https: [
  #         ...,
  #         port: 443,
  #         cipher_suite: :strong,
  #         keyfile: System.get_env("SOME_APP_SSL_KEY_PATH"),
  #         certfile: System.get_env("SOME_APP_SSL_CERT_PATH")
  #       ]
  #
  # The `cipher_suite` is set to `:strong` to support only the
  # latest and more secure SSL ciphers. This means old browsers
  # and clients may not be supported. You can set it to
  # `:compatible` for wider support.
  #
  # `:keyfile` and `:certfile` expect an absolute path to the key
  # and cert in disk or a relative path inside priv, for example
  # "priv/ssl/server.key". For all supported SSL configuration
  # options, see https://plug.hexdocs.pm/Plug.SSL.html#configure/1
  #
  # We also recommend setting `force_ssl` in your config/prod.exs,
  # ensuring no data is ever sent via http, always redirecting to https:
  #
  #     config :agent_lens, AgentLensWeb.Endpoint,
  #       force_ssl: [hsts: true]
  #
  # Check `Plug.SSL` for all available options in `force_ssl`.
end
